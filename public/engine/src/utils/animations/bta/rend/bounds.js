// src/utils/animations/bta/rend/bounds.js
// ponytail: convertido de ESM a script clasico. Expuesto via window.BTA_bounds.
// Dependencias leidas de window.BTA_utils.
function computeBounds(atlas, animation) {
  const u = window.BTA_utils || {};
  const getMX = u.getMX || (() => [1, 0, 0, 1, 0, 0]);
  const symbolLength = u.symbolLength || (() => 1);
...
if (typeof window !== "undefined") {
  window.BTA_bounds = { computeBounds };
}
