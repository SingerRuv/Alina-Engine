// src/utils/animations/bta/utils.js
// ponytail: convertido de ESM a script clasico. Funciones expuestas via window.BTA_utils
// (modulo puro, sin auto-registro en Phaser). Evita el import() dinamico que falla en Neutralino.
function stripBOM(s) {
...
function findFrameBlock(layer, frame) {
  const arr = layer.FR;
...
function totalFrames(animation) {
  let max = 0;
...
if (typeof window !== "undefined") {
  window.BTA_utils = { stripBOM, getMX, blendFromB, findFrameBlock, symbolLength, totalFrames };
}
