// src/utils/animations/bta/parser.js
// ponytail: convertido de ESM a script clasico. Funciones expuestas via window.BTA_parser.
// Dependencias leidas de window.BTA_utils (modulo puro, sin auto-registro en Phaser).
function buildAtlas(image, sm, anim) {
  const u = window.BTA_utils || {};
  if (typeof sm === "string") {
    try {
      sm = JSON.parse(u.stripBOM ? u.stripBOM(sm) : sm);
    } catch (e) {
  ...
  if (typeof anim === "string") {
    try {
      anim = JSON.parse(u.stripBOM ? u.stripBOM(anim) : anim);
    } catch (e) {
  ...
  return {
    ...
    totalFrames: u.totalFrames ? u.totalFrames(anim) : 0,
  };
}

function getAnimations(atlas) {
  ...
  if (layersWithSI.length === 1 && bestSpans.length >= 2) {
    for (const sp of bestSpans) out.push({ name: sp.name, type: "label", start: sp.start, duration: sp.duration });
    return out;
  }
  ...
  if (layersWithSI.length >= 2) {
    ...
    for (const sn of refs) {
      ...
      seen.add(sn);
      const len = u.symbolLength ? u.symbolLength(s) : 1;
      if (len < 2) continue;
...
function getFrameCount(atlas, animation) {
  return animation ? animation.duration | 0 : atlas.totalFrames;
}

function getFps(atlas) {
  return atlas.fps;
}

if (typeof window !== "undefined") {
  window.BTA_parser = { buildAtlas, getAnimations, getFrameCount, getFps };
}
