...
class Controls {
  static init() {
    ...
    if (this._fKeyBlocker) {
      window.removeEventListener("keydown", this._fKeyBlocker, true);
    }
    this._fKeyBlocker = (e) => {
    ...
    window.addEventListener("keydown", this._fKeyBlocker, { capture: true });
    const defaultPC = {
      // UI navigation
      UI_UP: [38, 87],
      UI_DOWN: [40, 83],
    ...
    this.PCKeyBinds = {};
    const savedGP = JSON.parse(localStorage.getItem("alina_controls_gp"));
