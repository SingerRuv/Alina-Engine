...
class FileSystem {
  ...
  // ponytail: atajos para escribir/leer archivos del proyecto (fuera de mods/basePath).
  // Solo disponibles en desktop Neutralino; en web el provider es null y se rompe con error claro.
  static async writeProjectText(fullPath, content) {
    if (!this.provider || typeof this.provider.writeProjectText !== "function") {
      throw new Error("FileSystem.writeProjectText requiere entorno desktop (Neutralino).");
    }
    return await this.provider.writeProjectText(fullPath, content);
  }
  static async readProjectText(fullPath) {
    if (!this.provider || typeof this.provider.readProjectText !== "function") {
      throw new Error("FileSystem.readProjectText requiere entorno desktop (Neutralino).");
    }
    return await this.provider.readProjectText(fullPath);
  }
}
