...
class CharacterRenderer extends Phaser.GameObjects.Sprite {
  constructor(scene, charId, role) {
    ...
    if (texture && texture.key !== "__MISSING") {
      ...
      // ponytail: la textura del atlas llega async. Cuando llegue, setear frame y bailar.
      scene.textures.once("addtexture", (key) => {
        if (key !== cacheKey) return;
        const t = scene.textures.get(cacheKey);
        if (t && t.key !== "__MISSING") {
          const firstFrame = t.getFrameNames()[0];
          if (firstFrame) this.setFrame(firstFrame);
          this.dance();
        }
      });
  ...
  getStageConfig(scene, role) {
    ...
    // ponytail: stages legacy (props[]) guardan los personajes en top-level characters.<charId>
    // con position/zIndex/cameraOffsets. Sin esto, el personaje queda en (0,0) y la camara
    // enfoca la esquina superior izquierda.
    if (stageData && stageData.characters && this.charId && stageData.characters[this.charId]) {
      const ch = stageData.characters[this.charId];
      return {
        position: ch.position || [0, 0],
        scale: 1,
        flip_x: false,
        layer: ch.zIndex !== undefined ? ch.zIndex : 5,
        scrollFactor: 1,
        opacity: 1,
        visible: true,
        camera_Offset: ch.cameraOffsets || ch.camera_Offset || [0, 0],
      };
    }
  ...
  applyStageConfig() {
    ...
    const charFlip = (this.charData && this.charData.image && this.charData.image.flip_x) === true ||
                     (this.charData && this.charData.flipX) === true;
  ...
  playAnim(animName, force = false) {
    ...
    const animConfig = this.charData.animations.find(a => (a.name || a.anim) === animName);
  ...
  playSingAnim(direction, isMiss = false, sustainLength = 0) {
    ...
    if (!isMiss && sustainLength > 0) {
        ...
        if (this.charData.animations.some(a => (a.name || a.anim) === loopAnimName)) {
            animName = loopAnimName;
        }
    }
    
    // Fallback a sing normal si el loop no existe
    if (!this.charData.animations.some(a => (a.name || a.anim) === animName)) {
  ...
  dance() {
    ...
    // LÓGICA DE REPOSO: No interrumpir idle (para que termine su ciclo natural).
    // ponytail: danceLeft/danceRight SÍ se alternan en cada beat (FNF); antes el guard
    // las bloqueaba y GF se quedaba en la primera sin alternar.
    if (this.anims.isPlaying) {
      const curKey = this.anims.currentAnim ? this.anims.currentAnim.key : "";
      if (curKey.endsWith("_idle")) {
        return; 
      }
    }

    const anims = this.charData.animations || [];
    const hasLeft = anims.some(a => (a.name || a.anim) === "danceLeft");
    const hasRight = anims.some(a => (a.name || a.anim) === "danceRight");
