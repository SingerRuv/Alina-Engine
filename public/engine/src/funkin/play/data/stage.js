...
class Stage {
  ...
  static loadAssets(scene, data, stageName) {
    const normalized = window.StageSchemaBridge
      ? window.StageSchemaBridge.normalize(data, stageName)
      : { data: data || {}, folder: (data && data.pathName) || stageName };
    const stageData = normalized.data;
    const folder = normalized.folder;
    if (
      stageData.background &&
      typeof stageData.background === "string" &&
      !stageData.background.startsWith("#")
    ) {
      window.StageImages.preload(scene, folder, { namePath: stageData.background });
    }
    const elements = stageData.stage || [];
  ...
  constructor(scene) {
    ...
    const raw = this.scene.cache.json.get("stageData_" + this.stageName) || {};
    // ponytail: el schema-bridge cubre ambos shapes (props legacy + stage actual).
    const normalized = window.StageSchemaBridge
      ? window.StageSchemaBridge.normalize(raw, this.stageName)
      : { data: raw, folder: raw.pathName || (raw._isFallback ? "mainStage" : this.stageName) };
    this.data = normalized.data;
    this.folder = normalized.folder;
    this.elements = [];
