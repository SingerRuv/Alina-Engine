  ...
  static loadCharacterAssets(scene, charId, data) {
    ...
    const isBTA =
      ...
      (data.renderType === "multianimateatlas" ||
        (typeof data.assetPath === "string" && data.assetPath.startsWith("shared:")));

    if (isBTA) {
      const atlasPng = `${window.Path.chars}${charId}/atlas.png`;
      const atlasXml = `${window.Path.chars}${charId}/atlas.xml`;
      const cacheKey = `char_atlas_${charId}`;
      if (!scene.textures.exists(cacheKey)) {
        scene.load.atlasXML(cacheKey, atlasPng, atlasXml);
      }
      return;
    }
