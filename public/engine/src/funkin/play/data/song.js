...
class Song {
  ...
  onSongEnd() {
    ...
    try {
      ...
      // CAMPAIGN (Story Mode): encadenar canciones de la week
      if (this.origin === "storymode") {
        const scene = this.scene;
        const data = scene.registry.get("playLoadData") || {};
        const playlist = data.Playlist || [];
        const scoreLogic = scene.referee && scene.referee.scoreLogic
          ? scene.referee.scoreLogic.statsP1.score
          : 0;
        const nextIndex = (data.PlaylistIndex || 0) + 1;

        if (nextIndex < playlist.length) {
          // Siguiente canción de la week, misma dificultad
          scene.registry.set("playLoadData", {
            CurrentSong: playlist[nextIndex],
            Difficulty: data.Difficulty,
            SceneOrigin: "storymode",
            Playlist: playlist,
            PlaylistIndex: nextIndex,
            CampaignScore: (data.CampaignScore || 0) + scoreLogic,
        ...
        // Fin de la week
        const totalScore = (data.CampaignScore || 0) + scoreLogic;
        const campaignId = data.CampaignId || "";
        const diff = String(data.Difficulty || "normal").toUpperCase();
        if (campaignId) {
          try {
            const key = `genesis_levelscore_${campaignId}_${diff}`;
            const prev = parseInt(localStorage.getItem(key), 10) || 0;
            if (totalScore > prev) {
              localStorage.setItem(key, String(totalScore));
            }
          } catch (e) { /* persistencia opcional */ }
        }
        const target = "StoryMenuScene";
        if (window.transitionTo) {
          window.transitionTo(scene, target);
        } else {
