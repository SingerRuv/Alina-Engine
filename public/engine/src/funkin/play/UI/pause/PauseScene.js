...
class PauseScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Discord RPC: en pausa
    if (window.DiscordRPC && typeof window.DiscordRPC.setPaused === "function") {
      const songName = this.referee && this.referee.scene && this.referee.scene.playData
        ? (this.referee.scene.playData.songName || this.referee.scene.playData.songId || "")
        : "";
      window.DiscordRPC.setPaused(songName);
    }
  ...
  handleAction(action) {
    switch (action) {
      ...
      case "exit":
        ...
        if (this.referee.song) this.referee.song.shutdown();
        this.scene.stop();
        const origin = this.referee.scene.playData.origin;
        const target =
          origin === "freeplay"
            ? "FreeplayScene"
            : origin === "storymode"
