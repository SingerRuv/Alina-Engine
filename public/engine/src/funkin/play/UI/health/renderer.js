...
class HealthRenderer {
  constructor(scene, logic) {
    ...
    // ponytail: colores de la barra calculados UNA VEZ en el constructor.
    // Prioridad: healthbar_colors (formato FNF, validando length>2) -> health.color -> default.
    ...
    const hasColor = (arr) => Array.isArray(arr) && arr.length > 2;
    let defaultUserColor = 0x66ff33; // verde lima (BF)
    let defaultOppColor = 0xff0000;  // rojo (Dad)
    this.userColor = hasColor(playerData?.healthbar_colors) ? toHex(playerData.healthbar_colors)
                   : playerData?.health?.color ? toHex(playerData.health.color)
                   : defaultUserColor;
    this.opponentColor = hasColor(opponentData?.healthbar_colors) ? toHex(opponentData.healthbar_colors)
                       : opponentData?.health?.color ? toHex(opponentData.health.color)
                       : defaultOppColor;
  ...
  update(time, delta) {
    ...
    this.barFillGraphics.fillStyle(this.opponentColor, 1);
    ...
    if (greenWidth > 0) {
      this.barFillGraphics.fillStyle(this.userColor, 1);
