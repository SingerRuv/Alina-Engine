...
class StrumlineLogic {
  ...
  handleInput(e, isDown) {
    ...
    // PR #7367: compensar la latencia de input. El songPosition ya incluye la
    // latencia del evento, así que se resta SIEMPRE (no solo en late hits).
    const inputLatencyMs =
      e && e.timeStamp !== undefined
        ? Math.max(0, performance.now() - e.timeStamp)
        : 0;
    ...
    this.dirs.forEach((dir) => {
      ...
      if (this.playerEnemy) {
        if (isP1) this.processInput(dir, isDown, true, false, null, null, inputLatencyMs);
        if (isP2 && this.twoPlayers) this.processInput(dir, isDown, false, false, null, null, inputLatencyMs);
      } else {
        if (isP1) this.processInput(dir, isDown, false, false, null, null, inputLatencyMs);
        if (isP2 && this.twoPlayers) this.processInput(dir, isDown, true, false, null, null, inputLatencyMs);
      }
    });
  }
  ...
  processInput(
    ...
    let actionTime =
      networkTime !== null
        ? networkTime
        : window.Conductor.songPosition - inputLatencyMs;
