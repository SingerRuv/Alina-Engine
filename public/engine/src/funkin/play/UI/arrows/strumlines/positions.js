...
if (isPlayer) {
  ...
  if (mode === "split") {
    if (index === 0 || index === 1) {
      x =
        this.STRUMLINE_X_OFFSET + this.cutoutSize.x / 2.5 + index * spacing + 100;
    } else {
      let rightStartX =
        width -
    ...
    let startX = this.STRUMLINE_X_OFFSET - 30 - 20;
    x = startX + index * spacing;
  } else {
...
if (isPlayer && mode === "mobile") {
  ...
  // Oponente arriba a la izquierda (estilo V-Slice).
  y = this.STRUMLINE_Y_OFFSET + offsets[1] + this.cutoutSize.y / 2 - 26;
