...
class BotLogic {
  ...
  hitPlayer(note, strumlines) {
    ...
    if (strum) {
      note.isBotPlay = true;
      // diff ≈ 20ms → rating "sick" (entre 12.5 killer y 45 sick threshold).
      // El skin no tiene "perfect", solo "sick" como mejor rating.
      strumlines.processHit(note, 20, strum, false);
    } else {
      note.destroy();
    }
  }
